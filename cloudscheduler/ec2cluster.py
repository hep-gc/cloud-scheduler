...
class EC2Cluster(cluster_tools.ICluster):
    ...
    def _get_connection(self):
        ...
        if self.cloud_type == "AmazonEC2":
            ...
            log.error("EC2Cluster don't know how to handle a %s cluster.", self.cloud_type)
    ...
    def vm_create(self, vm_name, vm_type, vm_user, vm_networkassoc,
        ...
        try:
            ...
            try:
                if e.errors and len(e.errors) > 0 and len(e.errors[0]) > 0 \
    ...
    def vm_poll(self, vm):
        ...
        try:
            ...
            if vm.spot_id:
                try:
                    ...
                    if spot_reservation.instance_id is None:
        ...
        with self.vms_lock:
            if instance and vm.status != self.VM_STATES.get(instance.state, "Starting"):
                ...
                vm.last_state_change = int(time.time())
                log.debug("VM: %s on %s. Changed from %s to %s.", vm.id, self.name, vm.status,
                          self.VM_STATES.get(instance.state, "Starting"))
    ...
    def vm_execwait(self, cmd, env=None):
        ...
        err = ""
        try:
            sub_p = Popen(cmd, shell=False,
                          stdout=subprocess.PIPE, stderr=subprocess.PIPE, env=env)
            if not utilities.check_popen_timeout(sub_p):
                (out, err) = sub_p.communicate(input=None)
            else:
                log.warning("Process %s timed out! cmd was %s", sub_p.pid, " ".join(cmd))
    ...
    """ Support Methods for spot pricing methods described above -
    methods can be used to answer spot pricing questions"""

    def _get_current_spot_price(self, image_type, instance_type, regions,
                                zones, filter_type, response_type, connection):
        """API function
    ...
    def _create_request(self, req_type, filter_type, response_type, other_args, args):
        if req_type == 'current_spot_price' or req_type == 'list_all_spot_price' or \
       req_type == 'periodic_spot_price':
            msg = {}
    ...
    def __get_current_spot_price(self, json_req):
