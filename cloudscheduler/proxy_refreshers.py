...
class JobProxyRefresher(threading.Thread):
    ...
    def __init__(self, job_pool):
        ...
        self.polling_interval = config_val.getint('global', 'job_proxy_refresher_interval')
    ...
    def run(self):
        try:
            ...
            while not self.quit:
                    ...
                    if job.is_proxy_expired():
                        ...
                        if job.get_myproxy_creds_name() != None:
                           ...
                           job.get_myproxy_server_port(), job.get_renew_time()):
                            ...
                            # credentials.
                            log.debug("Not renewing proxy for job %s because missing MyProxy info.",
                                      job.id)
                    else:
                ...
                log.verbose("Job proxy refreshing cycle done. [%d;%s;%s;%s]" %
...
class VMProxyRefresher(threading.Thread):
    ...
    def __init__(self, cloud_resources):
        ...
        self.heart_beat = time.time()
        # proxy expiry time poll interval, in seconds
        self.polling_interval = config_val.getint('global', 'vm_proxy_refresher_interval')
        ...
        try:
            ...
            while not self.quit:
                ...
                for vm in vms:
                    ...
                    if vmcertextime:
                        log.verbose("Proxy for VM %s expires in %s",
                                    vm.id, vmcertextime - datetime.datetime.utcnow())
                    if vm.is_proxy_expired():
                        log.warning("Proxy for VM %s is expired. \
                        ...
                        if vm.get_myproxy_creds_name() != None:
                           ...
                           vm.get_myproxy_creds_name(), vm.get_myproxy_server(), \
                           vm.get_myproxy_server_port(), vm.get_renew_time()):
                                # Yay, proxy renewal worked! :-)
                            ...
                            # credentials.
                            log.debug("Not renewing proxy for VM %s because missing MyProxy info.",
                                      vm.id)
                    else:
                ...
                # Lets record the current time and then log how much time the cycle took.
                cycle_end_ts = datetime.datetime.today()
                log.verbose("VM proxy refreshing cycle done. [%d;%s;%s;%s]",
...
class MyProxyProxyRefresher(object):
    ...
    def renew_proxy(self, proxy_file_path, myproxy_creds_name,
        ...
        if myproxy_server is None:
            log.warning("MyProxy credential name given but missing MyProxy server host. \
        ...
        myproxy_command = config_val.get('global', 'myproxy_logon_command')
